              ...
              <b-row>
                ...
                >
                 ...
                 <b-button
                    ...
                    >
                  <span class="text-nowrap">Add Speciality</span>
                </b-button>
            ...
            >
              ...
              <template #cell(actions)="data">
                ...
                >
                  ...
                  <b-dropdown-item :to="{ name: 'apps-speciality-view', params: { id: data.item.id } }">
                    <feather-icon icon="FileTextIcon" />
                    <span class="align-middle ml-50">Details</span>
                  </b-dropdown-item>
                  
                  <b-dropdown-item :to="{ name: 'apps-speciality-edit', params: { id: data.item.id } }">
...
export default {
  ...
  data(){
    return{
      ...
      items: [],
      currentPage: 1,
      perPage: this.$store.state.appConfig.specialtyListPerPage,
      rows: null,
      fields : [
        ...
        { key: 'field_en', label : 'Field EN' },
        { key: 'field_ar', label : 'Field AR' },
        { key: 'type', label : 'type' },
  ...
  methods:{
    redirectToAddForm(){
       router.replace('/apps/speciality/add')
    },
    deleteItem(id=0){
            
      axios.delete(`specialty/destroy/${id}`, 
        {
    ...
    getRecords(){
      this.isLoading = true
      this.isBusy = true
      let filterType = this.type == 'All' ? '' : this.type
      var url = `specialty/index?paginate=${this.perPage}&page=${this.currentPage}&type=${filterType}`
      ...
      .then(response => {
         ...
         console.log('specialty/index:' , response.data.data )
         ...
         for( let i=0 ; i< data.length ; i++ ){
           this.items.push(
             { 
               ...
               field_en : data[i].consultation_field ? data[i].consultation_field.title_en : '-' ,
               field_ar : data[i].consultation_field ? data[i].consultation_field.title_ar : '-' ,
               type : data[i].type ,
               status : data[i].status ? 'Active' : 'Inactive' ,
    ...
    paginateChanged(paginate){
      this.$store.state.appConfig.specialtyListPerPage = paginate
      this.currentPage = 1
      this.isBusy = true
