      ...
      <b-card title="Lessons List" > 
              ...
              <b-row>
                ...
                >
                  <div class="d-flex align-items-center justify-content-end">
                    ...
                    >
                      <span class="text-nowrap">Add Lesson</span>
                    </b-button>
                  </div>
                </b-col>
            ...
            >
              ...
              <template #cell(title)="data">
                  {{ data.item.title.length > 25 ? data.item.title.substring(0, 25) + '.....' : data.item.title }}
              </template>

              <template #cell(title_ar)="data">
                  {{ data.item.title_ar.length > 25 ? data.item.title_ar.substring(0, 25) + '.....' : data.item.title_ar }}
              </template>

              <template #cell(course)="data">
                  {{ data.item.course.length > 25 ? data.item.course.substring(0, 25) + '.....' : data.item.course }}
              </template>

              <template #cell(course_ar)="data">
                  {{ data.item.course_ar.length > 25 ? data.item.course_ar.substring(0, 25) + '.....' : data.item.course_ar }}
              </template>
              ...
              <template #cell(actions)="data">
                ...
                >
                  ...
                  <!-- <b-dropdown-item class="d-none" :to="{ name: 'apps-quizzes-add-lesson' , params: { id: data.item.id }}">
                    <feather-icon icon="FileTextIcon" />
                    <span class="align-middle ml-50">Add Quiz</span>
                  </b-dropdown-item> -->

                  <b-dropdown-item :to="{ name: 'apps-lessons-view',  params: { id: data.item.id } }">
                    <feather-icon icon="EditIcon" />
                    <span class="align-middle ml-50">Details</span>
                  </b-dropdown-item>
                  
                  <b-dropdown-item :to="{ name: 'apps-lessons-edit', params: { id: data.item.id } }">
                    <feather-icon icon="EditIcon" />
...
export default {
  ...
  data(){
    return{
      ...
      items: [],
      currentPage: 1,
      perPage: this.$store.state.appConfig.lessonListPerPage,
      rows: null,
      fields : [
        { key: 'id', label : 'id' },
        ...
        { key: 'course', label : 'Course EN' },
        { key: 'course_ar', label : 'Course AR' },
  ...
  methods:{
    redirectToAddForm(){
      router.push({ path: '/apps/lessons/add'})
    },
    deleteItem(id=0){
    ...
    getRecords(){
      this.isBusy = true
      this.isLoading = true
      var url = `lessons?paginate=${this.perPage}&page=${this.currentPage}`
      url +=  this.searchParameter != '' ? `&search[title]=${this.searchParameter}` : ''
      ...
      .then(response => {
         ...
         this.rows= response.data.lessons.total
         this.perPage=response.data.lessons.per_page
         this.dataMeta.from = response.data.lessons.from
         this.dataMeta.to = response.data.lessons.to
         this.dataMeta.of = response.data.lessons.total
         console.log('drows and per_page: ' , this.rows , ' ' , this.perPage )
         this.items = [];
         for( let i=0 ; i< data.length ; i++ ){
          let provider = ''
          if( data[i].course ){
            provider = data[i].course.user ? data[i].course.user.username : 'Nurabi'
          }else{
            provider = ''
          }

           let duration = ''
           if( data[i].course && data[i].course.is_live && data[i].session_duration != null ){
            duration = data[i].session_duration
           } else{
           ...
           this.items.push(
             { 
               ...
               course_ar : data[i].course ? data[i].course.title_ar : '',
               duration : duration,
    ...
    paginateChanged(paginate){
      this.$store.state.appConfig.lessonListPerPage = paginate
      this.currentPage = 1
      this.isBusy = true
