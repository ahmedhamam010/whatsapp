      ...
      <b-card title="Bank Questions List" > 
            ...
            >
              ...
              <template #cell(course)="data">
                <p :title="data.item.course">{{ data.item.course | truncate(20) }}</p>
              </template>
              ...
              <template #cell(actions)="data">
                ...
                >
                  ...
                  <b-dropdown-item :to="{ name: 'apps-bank-questions-reply-details', params: { id: data.item.id } }">
                    <feather-icon icon="FileTextIcon" />
                    <span class="align-middle ml-50">Details</span>
                  </b-dropdown-item>

                   <b-dropdown-item :to="{ name: 'apps-bank-questions-admin-edit',  params: { id: data.item.id , redirect : 'answeredList' } }">
...
export default {
  ...
  data(){
    return{
      ...
      items: [],
      currentPage: 1,
      perPage: this.$store.state.appConfig.userAnsweredBankQuestionListPerPage,
      rows: null,
      fields : [
        { key: 'id' },
        { key: 'question', label : 'question en' , thStyle: { width: "20%" }, },
        { key: 'question_ar', label : 'question ar' , thStyle: { width: "20%" }, },
  ...
  methods:{
    redirectToAddForm(){
       router.replace('/apps/categories/add')
    },
    deleteItem(id=0){
    ...
    getRecords(){
      this.isBusy = true
      this.isLoading = true
      var url = `bank-questions/index/yes?is_nurabi=0&paginate=${this.perPage}&page=${this.currentPage}`
      ...
      .then(response => {
         console.log('pending questions list:' , response.data.data.data )
        ...
        //  console.log('drows and per_page: ' , this.rows , ' ' , this.perPage )
         ...
         for( let i=0 ; i< data.length ; i++ ){
           let questioncourse = data[i].course ? data[i].course.title : '-' 
           let provider = ''
            if( data[i].course ){
           ...
           this.items.push(
             { 
               ...
               provider,
               status : data[i].is_show ? 'Published' : 'Unpublished' ,
    ...
    paginateChanged(paginate){
      this.$store.state.appConfig.userAnsweredBankQuestionListPerPage = paginate
      this.currentPage = 1
      this.isBusy = true
