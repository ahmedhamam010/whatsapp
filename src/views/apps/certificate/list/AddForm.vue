      ...
      <b-col md="8">
        <b-card>
            <template v-if="preview">
              <div class="position-relative img-preview-container m-auto" id="img-preview-container" style="width:665px;height:500px" >
                  <p class="position-absolute titleText invisible" id="titleText" >{{ title }}</p>
                  <p class="position-absolute bodyText invisible" id="bodyText">{{ body }}</p>
                  <p class="position-absolute dateText invisible" id="dateText" >{{ date }}</p>
                  <img :src="preview" id="preview-img" class="img-fluid invisible" style="width:665px;height:500px;" />
              </div>  
            </template>
        </b-card>
      </b-col>
...
export default {
  ...
  methods: {
    previewImage: function(event) {
       ...
       document.getElementById("titleText").classList.remove("invisible");
       document.getElementById("bodyText").classList.remove("invisible");
       document.getElementById("dateText").classList.remove("invisible");
       document.getElementById("preview-img").classList.remove("invisible");
    ...
    addCertificate: function () {
      this.$refs.simpleRules.validate().then(success => {
        if (success) {
          ...
          formData.append("image_width", 665 )
        ...
        //   formData.append("name_ar", this.name_ar )
          ...
          if( seoImage.files[0] ){
           ...
           axios
            .post(`certificates`, formData, this.headersObj)
            .then(response => {
