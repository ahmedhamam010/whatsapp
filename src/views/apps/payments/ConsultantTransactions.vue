        ...
        >
         ...
         <!-- Column: Actions -->
            <template #cell(actions)="data">
                ...
                :to="{ name: 'apps-payments-consultant-transaction-view', params: { id: data.item.user_id } }"
                >
...
export default {
  ...
  data(){
    return{
     items: [],
     currentPage: 1,
     perPage: this.$store.state.appConfig.paymentTransactionListPerPage,
     rows: null,
     fields : [
  ...
  methods:{
    getRecords(){
        ...
        this.isBusy = true
        axios
          .get(`get-list-user?paginate=${this.perPage}&page=${this.currentPage}`)
          .then(response => {
    ...
    paginateChanged(paginate){
      this.$store.state.appConfig.paymentTransactionListPerPage = paginate
      this.currentPage = 1
      this.isBusy = true  
