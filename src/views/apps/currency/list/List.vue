              ...
              <b-row>
                ...
                >
                  <div class="d-flex align-items-center justify-content-end">
                    ...
                    >
                      <span class="text-nowrap">Add Currency</span>
                    </b-button>
                  </div>
                </b-col>
            ...
            >
              ...
              <template #cell(actions)="data">
                ...
                >
                  ...
                  <!-- <b-dropdown-item :to="{ name: 'apps-currency-view', params: { id: data.item.id } }">
                    <feather-icon icon="FileTextIcon" />
                    <span class="align-middle ml-50">Details</span>
                  </b-dropdown-item> -->
                  
                  <b-dropdown-item :to="{ name: 'apps-currency-edit', params: { id: data.item.id } }">
...
export default {
  ...
  data(){
    return{
      ...
      items: [],
      currentPage: 1,
      perPage: this.$store.state.appConfig.currencyListPerPage,
      rows: null,
      fields : [
        { key: 'id',label: 'id' },
        { key: 'name',label: 'name' },
  ...
  methods:{
    redirectToAddForm(){
      router.replace('/apps/currency/add')
    },
    deleteItem(id=0){
    ...
    getRecords(){
      this.isBusy = true
      this.isLoading = true
      var url = `currencies?paginate=${this.perPage}&page=${this.currentPage}`
      ...
      .then(response => {
         console.log('lessons:' , response.data.languages )
         let data = response.data.languages.data;
         this.rows= response.data.languages.total
         this.perPage=response.data.languages.per_page
         this.dataMeta.from = response.data.languages.from
         this.dataMeta.to = response.data.languages.to
         this.dataMeta.of = response.data.languages.total
    ...
    paginateChanged(paginate){
      this.$store.state.appConfig.currencyListPerPage = paginate
      this.currentPage = 1
      this.isBusy = true
