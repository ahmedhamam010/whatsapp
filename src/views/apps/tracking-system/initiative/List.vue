              ...
              <b-row>
                ...
                >
                  <div class="d-flex align-items-center justify-content-end">
                    ...
                    >
                      <span class="text-nowrap">Add Initiative</span>
                    </b-button>
                  </div>
                </b-col>
            ...
            >
              ...
              <template #cell(actions)="data">
                ...
                >
                  ...
                  <b-dropdown-item :to="{ name: 'apps-tracking-system-initiative-view', params: { id: data.item.id } }">
                    <feather-icon icon="FileTextIcon" />
                    <span class="align-middle ml-50">Details</span>
                  </b-dropdown-item>
                  
                  <b-dropdown-item :to="{ name: 'apps-tracking-system-initiative-edit', params: { id: data.item.id } }">
...
export default {
  ...
  data(){
    return{
      items: [],
      currentPage: 1,
      perPage: this.$store.state.appConfig.TrackingSystemListInitiativePerPage,
      rows: null,
      fields : [
        { key: 'id', label : 'id' },
        ...
        { key: 'enrolled_numbers' , label: 'number of users' },
  ...
  methods:{
    redirectToAddForm(){
       router.push({ path: '/apps/trackingSystem/initiative/add'})
    },
    deleteItem(id=0){
          
      var formData = new FormData();
      formData.append("id",id )    
      axios.post(getDomain('no') + `trackingsystem/destroy?type=initiative`, formData)
        .then(response => {
        this.$toast({
    ...
    confirmDeleteMsg(id) {
      ...
      this.$bvModal
      .msgBoxConfirm('Are you sure you want to delete this initiative ?', {
    ...
    getRecords(){
      this.isBusy = true
      this.isLoading = true
      var url = getDomain('no') + `trackingsystem/index?flag=dash&type=initiative&paginate=${this.perPage}&page=${this.currentPage}`
      console.log(url)
      ...
      .then(response => {
         console.log('initiative/dasboardIndex:' , response.data.data )
         ...
         for( let i=0 ; i< data.length ; i++ ){
           this.items.push(
             { 
               ...
               enrolled_numbers : data[i].enrolled_numbers ,
