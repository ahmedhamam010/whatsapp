...
export default {
  ...
  methods: {
    previewImage: function(event,type) {
        ...
        reader.onload = (e) => {
          this.seoPreview = e.target.result;
        }
    ...
    fectchRecord:function(){
        ...
        this.type_id = record.parent ? record.parent : 'parent'  
        ...
        this.seoPreview = record.seo_image
    ...
    addForm: function () {
      this.$refs.simpleRules.validate().then(success => {
        if (success) {
          ...
          axios
            .post(getDomain('no')+`trackingsystem/fieldsGoals/update/${this.id}`, formData)
            .then(response => {
    ...
    getFields(){
      ...
      .then(response => {
        ...
        for( let s=0 ; s< response.data.data.length; s++  ){
            this.typeOptions.push( {label: response.data.data[s].name_en  , value: `${response.data.data[s].id}` })
        }
      })
      .catch(error => {
