...
export default {
  ...
  mounted(){
    ...
    .get(url)
    .then(response => {
   console.log( 'role record ' , response.data )
   this.name = response.data.data.role.name
    
    var listedPermissions = Object.values(response.data.data.rolePermissions)
   var permissionsList = [];
    for( let l=0 ; l< listedPermissions.length; l++  ){
      console.log('listedPermissions[l]' , listedPermissions[l])
        permissionsList.push(listedPermissions[l])
    }
    ...
    submitForm: function () {
        ...
        if (success) {
         ...
         this.isLoading = true 
          ...
          axios
            .put(`roles/${this.id}`, formData, this.headersObj)
            .then(response => {
