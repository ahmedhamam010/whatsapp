...
export default {
  ...
  data(){
    return{
      ...
      items: [],
      currentPage: 1,
      perPage: this.$store.state.appConfig.roleListPerPage,
      rows: null,
      fields : [
        { key: 'id',label: 'id' },
        { key: 'name',label: 'name' },
  ...
  methods:{
    ...
    getRecords(){
      this.isLoading = true
      this.isBusy = true
      var url = `roles?paginate=${this.perPage}&page=${this.currentPage}`
    ...
    paginateChanged(paginate){
      this.$store.state.appConfig.roleListPerPage = paginate
      this.currentPage = 1
      this.isBusy = true
