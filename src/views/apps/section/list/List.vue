...
export default {
  ...
  data(){
    return{
      ...
      items: [],
      currentPage: 1,
      perPage: this.$store.state.appConfig.sectionListPerPage,
      rows: null,
      fields : [
        { key: 'id', label : 'id' },
  ...
  methods:{
    redirectToAddForm(){
      router.push({ path: '/apps/sections/add'})
    },
    deleteItem(id=0){
    ...
    confirmDeleteMsg(id) {
      ...
      this.$bvModal
      .msgBoxConfirm('This Section is related to course and maybe has related lessons ,please confirm that you want to delete this record.', {
    ...
    getRecords(){
      this.isBusy = true
      this.isLoading = true
      var url = `sections?paginate=${this.perPage}&page=${this.currentPage}`
      ...
      .then(response => {
         ...
         let data = response.data.sections.data;
         this.rows= response.data.sections.total
         this.perPage=response.data.sections.per_page
         this.dataMeta.from = response.data.sections.from
         this.dataMeta.to = response.data.sections.to
         this.dataMeta.of = response.data.sections.total
    ...
    paginateChanged(paginate){
      this.$store.state.appConfig.sectionListPerPage = paginate
      this.currentPage = 1
      this.isBusy = true
