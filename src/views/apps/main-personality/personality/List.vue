<template>
  <div class="position-relative">   
      ...
      <b-card title="Personality Tests List" > 
              ...
              <b-row>
                ...
                >
                  <div class="d-flex align-items-center justify-content-end">
                    ...
                    >
                      <span class="text-nowrap">Add Personality Test</span>
                    </b-button>
                  </div>
                </b-col>
            ...
            >
              ...
              <template #cell(actions)="data">
                ...
                >
                  ...
                  <b-dropdown-item :to="{ name: 'apps-personality-test-view', params: { id: data.item.id } }">
                    <feather-icon icon="FileTextIcon" />
                    <span class="align-middle ml-50">Details</span>
                  </b-dropdown-item>
                  
                  <b-dropdown-item :to="{ name: 'apps-personality-test-edit', params: { id: data.item.id } }">
...
export default {
  ...
  data(){
    return{
      ...
      items: [],
      currentPage: 1,
      perPage: this.$store.state.appConfig.personalityTestListPerPage,
      rows: null,
      fields : [
        { key: 'id', label : 'id' },
        { key: 'title', label : 'title' },
        { key: 'title_ar', label : 'title_ar' },
        { key: 'time_limit', label : 'time limit' },
        { key: 'questions_number', label : 'questions number' },
        { key: 'actions' },
  ...
  methods:{
    redirectToAddForm(){
      router.replace('/apps/personality/test/add')
    },
    deleteItem(id=0){
            
      axios.delete(`personalityTest/${id}`, 
        {
    ...
    getRecords(){
      this.isBusy = true
      this.isLoading = true
      var url = `personalityTest?paginate=${this.perPage}&page=${this.currentPage}`
      ...
      .then(response => {
         console.log('personalty tests:' , response.data.data )
         ...
         for( let i=0 ; i< data.length ; i++ ){
           this.items.push(
             { 
               id:data[i].id ,
               title : data[i].name ,
               title_ar : data[i].name_ar ,
               time_limit : data[i].time_limit ,
               questions_number : data[i].questions_number ? data[i].questions_number : '' ,  
             })
         }
    ...
    paginateChanged(paginate){
      this.$store.state.appConfig.personalityTestListPerPage = paginate
      this.currentPage = 1
      this.isBusy = true
