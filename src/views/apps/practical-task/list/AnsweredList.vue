      ...
      <b-card title="Practical Tasks List" > 
            ...
            >
              ...
              <template #cell(name)="data">
                  ...
                  {{ data.item.name.length > 25 ? data.item.name.substring(0, 25) + '.....' : data.item.name }}
              </template>
              ...
              <template #cell(actions)="data">
                ...
                >
                  ...
                  <b-dropdown-item :to="{ name: 'apps-pending-practical-tasks-view', params: { id: data.item.id , status : 'pending' } }">
                ...
                <b-button
                  ...
                  :to="{ name: 'apps-pending-practical-tasks-view', params: { id: data.item.id , status : 'answered' } }"
                >
...
export default {
  ...
  data(){
    return{
      ...
      items: [],
      currentPage: 1,
      perPage: this.$store.state.appConfig.answeredTaskListPerPage,
      rows: null,
      fields : [
        { key: 'id', label : 'id' },
        { key: 'name', label : 'name' },
  ...
  methods:{
    redirectToAddForm(){
      router.replace('/apps/practicalTasks/add')
    },
    deleteItem(id=0){
    ...
    getRecords(){
      this.isBusy = true
      this.isLoading = true
      var url = `practicalTasksUser/null/users/notPending?paginate=${this.perPage}&page=${this.currentPage}`
      ...
      .then(response => {
         console.log('pending practical tasks:' , response.data.data )
         ...
         for( let i=0 ; i< data.length ; i++ ){
           this.items.push(
             { 
               ...
               practical_task : data[i].practical_task.question ,
               course : data[i].practical_task.course ? data[i].practical_task.course.title : '',
               course_ar : data[i].practical_task.course ? data[i].practical_task.course.title_ar : '',
    ...
    paginateChanged(paginate){
      this.$store.state.appConfig.answeredTaskListPerPage = paginate
      this.currentPage = 1
      this.isBusy = true
