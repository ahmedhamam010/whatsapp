<template>
  <div class="position-relative">   
      ...
      <b-card title="Practical Tasks List" > 
              ...
              <b-row>
                ...
                >
                  <div class="d-flex align-items-center justify-content-end">
                    ...
                    >
                      <span class="text-nowrap">Add Practical Task</span>
                    </b-button>
                  </div>
                </b-col>
            ...
            >
              ...
              <template #cell(question)="data">
                  ...
                  {{ data.item.question.length > 25 ? data.item.question.substring(0, 35) + '.....' : data.item.question }}
              </template>

              <template #cell(question_ar)="data">
                  
                  {{ data.item.question_ar.length > 25 ? data.item.question_ar.substring(0, 35) + '.....' : data.item.question_ar }}
              </template>
              ...
              <template #cell(actions)="data">
                ...
                >
                  ...
                  <b-dropdown-item :to="{ name: 'apps-practical-tasks-view', params: { id: data.item.id } }">
                    <feather-icon icon="FileTextIcon" />
                    <span class="align-middle ml-50">Details</span>
                  </b-dropdown-item>
                  
                  <b-dropdown-item :to="{ name: 'apps-practical-tasks-edit', params: { id: data.item.id } }">
...
export default {
  ...
  data(){
    return{
      ...
      items: [],
      currentPage: 1,
      perPage: this.$store.state.appConfig.practicalTaskListPerPage,
      rows: null,
      fields : [
        { key: 'id', label : 'id' },
  ...
  methods:{
    redirectToAddForm(){
      router.push({path: '/apps/practicalTasks/add' })
    },
    deleteItem(id=0){
            
      axios.delete(`practicalTasks/${id}`, 
        {
    ...
    getRecords(){
      this.isBusy = true
      this.isLoading = true
      var url = `practicalTasks?paginate=${this.perPage}&page=${this.currentPage}`
      ...
      .then(response => {
         console.log('practical tasks:' , response.data.practicalTasks )
         let data = response.data.practicalTasks.data;
         this.rows= response.data.practicalTasks.total
         this.perPage=response.data.practicalTasks.per_page
         this.dataMeta.from = response.data.practicalTasks.from
         this.dataMeta.to = response.data.practicalTasks.to
         this.dataMeta.of = response.data.practicalTasks.total
          ...
          if( data[i].course ){
           ...
           this.items.push(
             { 
               ...
               question_ar : data[i].question_ar ? data[i].question_ar : '' ,
               provider,
    ...
    paginateChanged(paginate){
      this.$store.state.appConfig.practicalTaskListPerPage = paginate
      this.currentPage = 1
      this.isBusy = true
