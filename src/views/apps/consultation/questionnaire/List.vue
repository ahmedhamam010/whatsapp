<template>
  <div class="position-relative">   
      ...
      <b-card title="Questionnaires List" > 
            ...
            <template #cell(defualt)="data">
              <b-form-checkbox
                :checked="data.item.default ? 'true' : 'false'"
                name="check-button"
                switch
                inline
                @change="updateDefualt(data.item.id)"
                >
            ...
            <template #cell(actions)="data">
              ...
              >
                ...
                <b-dropdown-item :to="{ name: 'apps-consultation-questionnaire-view', params: { id: data.item.id } }">
                  <feather-icon icon="FileTextIcon" />
                  <span class="align-middle ml-50">Details</span>
                </b-dropdown-item>
                
                <b-dropdown-item :to="{ name: 'apps-consultation-questionnaire-edit', params: { id: data.item.id } }">
...
<script>
import {
  BButton, BMedia, BAvatar, BRow, BCol, BFormGroup, BFormInput, BForm, BTable, BCard, BCardHeader, BCardTitle, BDropdown , BFormTextarea, BDropdownItem, BCardText, BPagination ,BBadge , BSpinner , BFormCheckbox
} from 'bootstrap-vue'
...
export default {
  components: {
    ...
    BDropdown,
    BDropdownItem,
    BCardText, BPagination, vSelect, BBadge, BSpinner,BFormCheckbox
  ...
  data(){
    return{
      ...
      items: [],
      currentPage: 1,
      perPage: this.$store.state.appConfig.surveyListPerPage,
      rows: null,
      fields : [
        { key: 'id', label : 'id' },
  ...
  methods:{
    redirectToAddForm(){
      router.replace('/apps/consultation/questionnaire/add')
    },
    deleteItem(id=0){
      this.isBusy = true      
      axios.delete(`questionnaires/${id}`, 
        ...
        .catch(error => {
        this.isBusy = false
          console.log('error delete survey',error.response.data.msg)
        this.$toast({
          component: ToastificationContent,
          position: 'top-right',
          props: {
    ...
    updateDefualt(id){
       this.isBusy = true    
       this.isLoading = true 
      axios.put(`consultants/questionnaires/${id}/default`)
        .then(response => {
        this.$toast({
          component: ToastificationContent,
          position: 'top-right',
    ...
    getRecords(){
      this.isBusy = true
      this.isLoading = true
      var url = `consultants/questionnaires?paginate=${this.perPage}&page=${this.currentPage}`
      ...
      .then(response => {
         console.log('questionners:' , response.data.data )
         ...
         for( let i=0 ; i< data.length ; i++ ){
           this.items.push(
             { 
               ...
               status: data[i].status ? 'Published' : 'Drafted',
               default: data[i].model_default
             })
         }
         this.isBusy = false
         this.isLoading = false
    ...
    paginateChanged(paginate){
      this.$store.state.appConfig.surveyListPerPage = paginate
      this.currentPage = 1
      this.isBusy = true
