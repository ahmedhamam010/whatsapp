  ...
  <div class="position-relative">
    <b-row>
      <b-col cols="12">
        ...
        <b-card title="Edit Consultant Questionnaire">
            <div>
...
export default {
  ...
  methods: {
    ...
    removeQuestion(index){
        if( this.form.parent[index].id ){
          this.old_questions_ids.push(this.form.parent[index].id)
        }
    ...
    removeOption: function (index ,indexs) {
        if(this.form.parent[index].options.length > 1 ){
          if( this.form.parent[index].options[indexs].id ){
            this.old_options_ids.push(this.form.parent[index].options[indexs].id)
          }
          console.log( this.old_options_ids )
    ...
    submitForm: function () {
        ...
        if (success) {
           ...
           for( let i=0 ; i < questionsData.length ; i++ ){
            ...
            for( let q=0 ; q < this.old_questions_ids.length ; q++ ){
                formData.append(`old_questions_ids[${q}]`, this.old_questions_ids[q] )
            }

            for( let q=0 ; q < this.old_options_ids.length ; q++ ){
                formData.append(`old_options_ids[${q}]`, this.old_options_ids[q] )
            }
            

        axios
          .post(`consultants/questionnaires/${this.id}?_method=put`, formData)
          .then(response => {
            ...
            this.form.parent.push( 
              {
                ...
                'id':response.data.data.questionnaire_questions[q].id,
