        ...
        <b-tabs>
               ...
               <b-list-group-item>
                ...
                <b-list-group-item>
                  <b-row>
                    ...
                    <b-button
                      v-if="status == 'pending' || status == 'rejected' || userKindStatus == 'recieve'"
                    ...
                    &nbsp;
                    <b-button
                    v-if="status == 'pending' || userKindStatus == 'recieve' "
                    ...
                    <b-button
                    v-if=" status == 'accepted' && invitationId != 0 "
                      v-ripple.400="'rgba(234, 84, 85, 0.15)'"
                      variant="outline-danger"
                    >
                      <feather-icon
                        ...
                        class="mr-50"
                      />
                      <span class="align-middle" @click="removeAssistant">Remove</span>
                    </b-button>
          ...
          <b-tab @click.once="getConsultantSessions" v-if="adminLogged">
            ...
            <template v-if="items.length > 0">
              <b-table responsive :busy="isBusy" :items="items" class="mt-2"></b-table>
              <b-pagination
                v-model="currentPage"
            ...
            <template v-if="noRecords">
              <h4 class="text-center mt-3 mb-3">There Is No Data Till Now</h4>
            </template>
...
export default {
  ...
  data(){
    return{
      ...
      description_seo_ar:'',
      image_seo:'',
      userKindStatus: router.currentRoute.params.type,
      headersObj  : { 
          headers: 
            { 
      ...
      is_top:0,  
      invitationId: router.currentRoute.params.invitationId,   
      user_id:null,
      noRecords:false,
      isLoading:true
    }
  ...
  mounted(){
    ...
    axios
          ...
          console.log( 'this.consultationInfos/requests/218 ' , response.data.data.user )
          ...
          this.avatar = this.avatar ? response.data.data.user.avatar : this.defaultImg
  ...
  methods:{
    ...
    requestAsstistant(){     
        ...
        axios
        .post(url,{ invitations : [this.id]}, this.headersObj)
        .then(response => {
          this.$toast({
            component: ToastificationContent,
            position: 'top-right',
          ...
          router.replace('/apps/consultation/assitants/list')
        })
        .catch(error => {
        this.$toast({
          component: ToastificationContent,
    ...
    getConsultantSessions(){
      this.isLoading = true
      //consultationSessions?consultant_session=1&consultant_id=${this.id}&paginate=10&page=${this.currentPage}
      ...
      .then(response => {
        this.isLoading = false
         ...
         this.isBusy = false
         this.noRecords = this.items.length > 0  ? false : true
      } )
      .catch(error => {
    ...
    removeAssistant(){
     //remove assistant
      this.isLoading = true
      var statusMsg = 'Assistant Removed Successfully'; 
       axios
      .delete(`consultationInfos/invitations/${this.invitationId}`)
      .then(response => {
          this.isLoading = false
          this.$toast({
                component: ToastificationContent,
           ...
           router.replace(`/apps/consultation/requested/assitants/list`)
      } )
      .catch(error => {
              this.isLoading = false
              this.$toast({
