        ...
        <b-card title="Add Consultant">
            ...
            <validation-observer ref="simpleRules">
              ...
              <b-row>
                ...
                <!-- Item password -->
                    <b-col md="12" v-if="showPAsswordField">
                        <b-form-group
                        label="password *"
                        label-for="password"
                        >
                ...
                <b-col md="6">
                  ...
                  <!-- Seo Name EN -->
                    ...
                    <!-- File -->
                    <b-col md="12" class="qualification-container">
                      <b-row v-for="(item , index) in qualifications" :key="item" class="d-flex align-items-center">
                        <b-col sm="9">
...
<script>
import {
  BForm, BFormGroup, BFormInput, BRow, BCol, BButton, BFormCheckbox, BCard , BFormTextarea, BModal , VBModal, BListGroup, BListGroupItem,BFormFile,BInputGroup,BInputGroupAppend
} from 'bootstrap-vue'
...
export default {
  components: {
    ...
    BModal,
    VBModal,
    BListGroup, BListGroupItem,BFormFile,BInputGroup,BInputGroupAppend
  },
  directives: {
    'b-modal': VBModal,
    Ripple,
  ...
  mounted() {
    ...
    .then(response => {
       ...
       for( let u =0 ; u < users.length ; u++ ){
           this.userOptions.push({ label:users[u].username , value:users[u].id , provider_id:users[u].provider_id  })
       }
    })
    .then(
      this.fetchLanguages()
  ...
  methods: {
    ...
    submitForm: function () {
        ...
        if (success) {
         ...
         formDataInputs.append("specialty", this.specialty )
       ...
       const certificatesLength = document.querySelector('.qualification-container').childElementCount
       if( certificatesLength > 0 ){
        for( let cer=1 ; cer <= certificatesLength ; cer++ ){
          formDataInputs.append("certificates[]", document.querySelector('#certificate-' + cer ).files[0]);
        }
        
       }

       if( this.password ){
        formDataInputs.append("password", this.password )
       }
       
       

       console.log( 'this.specialty' , this.specialty )
         console.log( 'type of specialty' , typeof this.specialty )
          
        
          axios
            .post(`consultationInfos/requests`, formDataInputs, this.headersObj)
            .then(response => {
    ...
    addQualificationFile() {
      this.qualifications.push(this.qualificationCounter++);
    },
    removeQualificationFile(index) {
      this.qualifications.splice(index, 1);
    },
    checkProviderID(event){
      let selectedUserHasProviderId = this.userOptions.find( (user) => user.value == event ).provider_id
      this.showPAsswordField = selectedUserHasProviderId ? true : false
