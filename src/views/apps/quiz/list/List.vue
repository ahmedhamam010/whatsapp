            ...
            >
              ...
              <template #cell(actions)="data">
                ...
                >
                  ...
                  <b-dropdown-item :to="{ name: 'apps-quizzes-view-section', params: { id: data.item.id } }">
                    <feather-icon icon="FileTextIcon" />
                    <span class="align-middle ml-50">Details</span>
                  </b-dropdown-item>
                  
                  <b-dropdown-item :to="{ name: 'apps-quizzes-edit-section', params: { id: data.item.id } }">
...
export default {
  ...
  data(){
    return{
      ...
      items: [],
      currentPage: 1,
      perPage: this.$store.state.appConfig.quizListPerPage,
      rows: null,
      fields : [
        { key: 'id', label : 'id' },
  ...
  methods:{
    redirectToAddForm(){
      router.push({path: '/apps/quiz/add'})
    },
    deleteItem(id=0){
    ...
    getRecords(){
      this.isBusy = true
      this.isBusy = true
      var url = `quizzes/index/quiz?paginate=${this.perPage}&page=${this.currentPage}`
      ...
      .then(response => {
         console.log('lessons:' , response.data.quizzes )
         let data = response.data.quizzes.data;
         this.rows= response.data.quizzes.total
         this.perPage=response.data.quizzes.per_page
         this.dataMeta.from = response.data.quizzes.from
         this.dataMeta.to = response.data.quizzes.to
         this.dataMeta.of = response.data.quizzes.total
    ...
    paginateChanged(paginate){
      this.$store.state.appConfig.quizListPerPage = paginate
      this.currentPage = 1
      this.isBusy = true
